...
class UI:
    ...
    def draw_text(
        self, surface, text, font_size, color, x, y, align="left", max_width=None
    ):
        """
        ...
        if max_width:
            ...
            if rects:
                bounding_rect = (
                    rects[0].unionall(rects[1:]) if len(rects) > 1 else rects[0]
                )
    ...
    def draw_header(self, surface, title="VRA Interface", voice_assistant_active=False):
        ...
        header_rect = self.draw_panel(
            surface, 0, 0, SCREEN_WIDTH, Layout.HEADER_HEIGHT, bg_color=Colors.PRIMARY
        )
    ...
    def draw_navigation_panel(self, surface, navigating_to=None):
        ...
        nav_panel = self.draw_panel(
            ...
            Layout.CONTENT_Y,
            Layout.NAV_WIDTH,
            Layout.CONTENT_HEIGHT - Layout.FOOTER_HEIGHT,
            bg_color=Colors.LIGHT_GRAY,
    ...
    def draw_info_panel(self, surface, faq_manager, scroll_offset=0):
        ...
        if faq_manager.selected_question:
            ...
            scroll_button_x = (
                Layout.INFO_X + Layout.INFO_WIDTH - Layout.MARGIN - scroll_button_width
            )
            ...
            for i in range(start_index, end_index):
                ...
                q_button = self.draw_panel(
                    ...
                    - (Layout.MARGIN * 2)
                    - (
                        scroll_button_width + 5
                        if len(all_questions) > visible_count
                        else 0
                    ),  # Make space for scroll buttons
                    q_height,
                    bg_color=Colors.PRIMARY_LIGHT,
                    border_radius=5,
                )
            ...
            if len(all_questions) > visible_count:
                ...
                scroll_up_rect = pygame.Rect(
                    scroll_button_x, scroll_up_y, scroll_button_width, q_height
                )
                scroll_down_rect = pygame.Rect(
                    scroll_button_x, scroll_down_y, scroll_button_width, q_height
                )
                ...
                up_arrow_points = [
                    (
                        scroll_up_rect.centerx,
                        scroll_up_rect.top + scroll_up_rect.height * 0.2,
                    ),  # Top point
                    (
                        scroll_up_rect.left + scroll_up_rect.width * 0.2,
                        scroll_up_rect.top + scroll_up_rect.height * 0.7,
                    ),  # Bottom-left
                    (
                        scroll_up_rect.right - scroll_up_rect.width * 0.2,
                        scroll_up_rect.top + scroll_up_rect.height * 0.7,
                    ),  # Bottom-right
                ]
                ...
                # Down Arrow Button
                down_color = (
                    Colors.SECONDARY if end_index < len(all_questions) else Colors.GRAY
                )
                down_arrow_points = [
                    (
                        scroll_down_rect.centerx,
                        scroll_down_rect.top + scroll_down_rect.height * 0.8,
                    ),  # Bottom point
                    (
                        scroll_down_rect.left + scroll_down_rect.width * 0.2,
                        scroll_down_rect.top + scroll_down_rect.height * 0.3,
                    ),  # Top-left
                    (
                        scroll_down_rect.right - scroll_down_rect.width * 0.2,
                        scroll_down_rect.top + scroll_down_rect.height * 0.3,
                    ),  # Top-right
                ]
                pygame.draw.polygon(surface, down_color, down_arrow_points)
                scroll_down_button = (
                    scroll_down_rect  # Use the rect for click detection
                )

        return info_panel, question_buttons, scroll_up_button, scroll_down_button

    def draw_message_panel(
        self, surface, text, font_size="regular", bg_color=Colors.INFO
    ):
        """
    ...
    def draw_warning(self, surface, message="Warning: Obstacle detected!"):
        ...
        self.draw_text(
            ...
            align="left",
            max_width=warning_width
            - (icon_radius * 2 + 70),  # Max width considering icon and padding
        )
...
class RobotInterface:
    ...
    def __init__(self):
        ...
        self.is_showing_message = (
            False  # Renamed from show_custom_message to avoid name conflict
        )
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_ESCAPE:
                    ...
                    event.key == K_UP and not self.navigating_to
                ):  # Only allow selection if not navigating
                ...
                # ====> Based on what ever button is currently active, add lines to send data to ARDUINO
                if self.nav_buttons:
                    if self.navigating_to:
                        if self.nav_buttons.get("cancel_button") and self.nav_buttons[
                            "cancel_button"
                        ].collidepoint(pos):
                ...
                if not self.faq_manager.selected_question:
                    ...
                    scrolled = False
                    if (
                        self.faq_scroll_up_button
                        and self.faq_scroll_up_button.collidepoint(pos)
                    ):
                        if self.faq_scroll_offset > 0:
                            self.faq_scroll_offset -= 1
                            scrolled = True
                    elif (
                        self.faq_scroll_down_button
                        and self.faq_scroll_down_button.collidepoint(pos)
                    ):
                        total_questions = len(self.faq_manager.get_all_questions())
                        # Use stored/recalculated visible count
                        if (
                            self.faq_scroll_offset
                            < total_questions - self.faq_visible_count
                        ):
                            self.faq_scroll_offset += 1
                            scrolled = True
    ...
    def update(self):
        ...
        if (
            self.show_warning
            and time.time() - self.warning_time > self.warning_duration
        ):
            self.show_warning = False
    ...
    def draw(self):
        ...
        _, self.live_button = self.ui.draw_header(
            screen, "VRA Mobile Robot:", self.voice_assistant_running
        )

        # Draw either the message panel or navigation panel
        if self.is_showing_message:
            self.ui.draw_message_panel(
                screen, self.message_text, self.message_font_size, self.message_bg_color
            )
        else:
            self.nav_buttons = self.ui.draw_navigation_panel(screen, self.navigating_to)

        # Draw the FAQ panel
        _, self.faq_buttons, self.faq_scroll_up_button, self.faq_scroll_down_button = (
            self.ui.draw_info_panel(screen, self.faq_manager, self.faq_scroll_offset)
        )
